...
export default {
  props: ['todos', 'type'],
  methods: {
    handleTodoToggle(todo) { // eslint-disable-line
        ...
        update: (store, { data: { update_todos }}) => {
          if (update_todos.affected_rows) {
            if (this.type === 'private') {
              ...
              const toggleTodo = data.todos.find( i => i.id === todo.id)
    ...
    handleTodoDelete(todo) { // eslint-disable-line
      this.$apollo.mutate({
        ...
        update: (store, { data: { delete_todos }}) => {
          if(delete_todos.affected_rows) {
            if (this.type === 'private') {
              ...
              data.todos = data.todos.filter(i => {
                return i.id !== todo.id
              })
              store.writeQuery({
                query: GET_MY_TODOS,
